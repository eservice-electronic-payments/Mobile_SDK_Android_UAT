...
From the mobile app's point of view, essentially the requirement is to obtain a session token for connection with a back-end server infrastructure in order to (with help from the infrastructure) successfully guide the user through the process of completing a payment transaction.

The 3-D Secure (V2), as a standard, marks various data fields as mandatory for a compliant transaction, with some of this data being generated by the mobile device itself, but for some other data the responsibility may lie somewhere else, which can depend on a Merchant's use-case and all the decisions standing behide its implementation.

The data generated and provided to the back-end by the mobile device itself in our example looks as enlisted above in the code snippet. The paramteres are currently very likely to change in the near future following the implementation of the 3DS standard in version 2. The additional data fields required by the V2 standard are enlisted and can be found [here on the Wiki page](https://intelligentpayments.atlassian.net/wiki/spaces/IGP/pages/2325053569/Summary+of+Direct+API+changes+to+implement+3DS2).
​
​
