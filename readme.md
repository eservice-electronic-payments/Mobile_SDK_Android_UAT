...
From the mobile app's perspective, essentially the requirement is to obtain a session token for connection with a back-end server infrastructure in order to (with help from the infrastructure) successfully guide the user through the process of completing a payment transaction.
